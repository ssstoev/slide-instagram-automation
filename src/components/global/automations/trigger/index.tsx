...
const Trigger = ({ id }: Props) => {
  ...
  // return any active triggers if we have any
  if (data?.data && data?.data?.trigger.length > 0) {
    ...
    <div className='flex flex-col ga-y-6 items-center'>
      ...
      {/* if we have > 1 trigger for an automation we render the below part as well*/}
      {data.data.trigger.length > 1 
      && <>
        <div className='relative w-6/12 m-3 self-center'>
  ...
  // display only 'Add Trigger' button if we don't have any active triggers
  // PopOver we adjust the css based on that if we have selected triggers
